(function ($) {
	...
	windows.on('scroll', function () {
		...
		if (scroll < 200) {
			$('.home-other-navigation-menu').removeClass('d-none');
		}else{
    		$('.home-other-navigation-menu').addClass('d-none');
		}
	...
	//sidbar nav
	$('#sidebar ul.has-sub ').hide();
	$('#sidebar a.has-dropdown').on('click', function(e){
		if($(this).next('.has-sub').length > 0 ){
			$(this).next('.has-sub').slideToggle('slow');
...
/*-----
	...
	$("#newsletterBtn").click(function (e) {
		e.preventDefault();
		var email = $("#newsletterEemail").val();
		$.ajaxSetup({
			headers: {
		...
		$.ajax({
			type:'POST',
			url: '/sareasb20min/newsletter/store',
